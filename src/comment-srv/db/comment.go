...
func InsertHotComment(comment *entity.Comment) error {
	...
	_, err := db.Exec("INSERT INTO `comment`(`film_id`,`title`,`content`,`head_img`,`nick_name`,`create_at`) VALUES (?,?,?,?,?,?)", comment.FilmId, comment.Title, comment.Content, comment.HeadImg, comment.NickName, today)
	return err
}
